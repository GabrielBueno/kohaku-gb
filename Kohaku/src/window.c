...
static void handle_key_event(struct window *win, SDL_Event *e);
...
static void handle_key_event(struct window* win, SDL_Event* e) {
    enum joypad_button btn;
    enum joypad_sate   state;
    ...
    case SDLK_DOWN:
        btn = JOYPAD_DPAD_DOWN;
        break;

    case SDLK_LEFT:
        btn = JOYPAD_DPAD_LEFT;
        break;

    case SDLK_RIGHT:
        btn = JOYPAD_DPAD_RIGHT;
        break;
    ...
    case SDLK_SPACE:
        btn = JOYPAD_BTN_SELECT;
        break;

    case SDLK_RETURN:
        btn = JOYPAD_BTN_START;
        break;
    ...
    joypad_set(&win->gb->joypad, btn, state);
}